...
int ask_me()
{
	...
	command("say ÕâÄã»¹ÓÃÎÊÎÒ£¿£¡");
	return 1;
//	return "ÕâÄã»¹ÓÃÎÊÎÒ£¿£¡";	// if already in wudidong, nopoint
    }
	...
	command("recruit "+me->query("id"));
	command("say ºÙºÙ£¬ÄãÈôÊÇ²»Ô¸£¬¿ÉÄª¹ÖÎÒĞÄºİÊÖÀ±£¡");
...
//	return "ºÙºÙ£¬ÄãÈôÊÇ²»Ô¸£¬¿ÉÄª¹ÖÎÒĞÄºİÊÖÀ±£¡";
	...
	command("say ·ñÔò¾Í±ğ¹ÖÎÒÉ±ÈËÃğ¿ÚÁË£¡");
    call_out("mie_kou", 60, me);		// will not transform back into
	return 1;
//    return "·ñÔò¾Í±ğ¹ÖÎÒÉ±ÈËÃğ¿ÚÁË£¡";		// human coz heneeds to kill
}
void transform()
{
  object weapon;
...
//    if (!interactive()) return;
	...
	command("say ²»´í£¬Ê¶Ê±ÎñÕßÎª¿¡½Ü£¡");	// don't kill him, compliment him
